...
def add_coords(atoms:Atoms,calc:CP2K):
    ...
    for i in range(len(atoms)):
        default_keyword.append(atoms[i].symbol+" "+" ".join(map(str, atoms.positions[i]*10))) # convert to angstroms
...
def add_cell(atoms:Atoms,calc:CP2K):
    ...
    if not atoms.cell:
       ...
       atoms.center()
       max_coords=atoms.positions.max(axis=1)*10 # convert to angstroms
       min_coords=atoms.positions.min(axis=1)*10 # convert to angstroms
