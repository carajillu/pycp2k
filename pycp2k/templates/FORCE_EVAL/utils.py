...
def add_SCF_OT(calc:CP2K,**kwargs):
    ...
    print(SCF.OT.Preconditioner)
    print(SCF.OT.Minimizer)
    #inner scf loop
    SCF.Scf_guess=kwargs.get("scf_guess","RESTART")
    SCF.Max_scf=kwargs.get("scf_max_scf",20)
    SCF.Eps_scf=kwargs.get("scf_eps_scf",1e-6)
    print(SCF.Scf_guess)
    print(SCF.Max_scf)
    print(SCF.Eps_scf)

    #outer scf loop
    SCF.OUTER_SCF.Max_scf=kwargs.get("outer_scf_max_scf",20)
    SCF.OUTER_SCF.Eps_scf=kwargs.get("outer_scf_eps_scf",1e-6)
    print(SCF.OUTER_SCF.Max_scf)
    print(SCF.OUTER_SCF.Eps_scf)
